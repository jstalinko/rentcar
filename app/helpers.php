...
function easyWa($to, $message)
{
    ...
    try {
        $response = \Illuminate\Support\Facades\Http::asForm()->post($url, $postFields);
        $result = $response->json();
        file_put_contents(storage_path('logs/easywa.log'), json_encode($result) . "\n\n", FILE_APPEND);
        return $result;
    } catch (\Exception $e) {
        file_put_contents(storage_path('logs/easywa.log'), "ERROR: " . $e->getMessage() . "\n\n", FILE_APPEND);
